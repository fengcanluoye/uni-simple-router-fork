...
export function rewriteMethod(router: Router): void {
      ...
      uni[name] = async function (
        ...
        if (originCall) {
          if (router.options.platform === 'app-plus') {
            await HomeNvueSwitchTab(
              router,
              params as navtoRule,
              cacheOldMethod['reLaunch'],
            )
          }
          uniOriginJump(
            router,
...
function callRouterMethod(
  ...
  if (funName === 'navigateBack') {
    ...
    if (
      getDataType<number | undefined>((option as uniBackApiRule).delta) ===
      '[object Number]'
    ) {
    ...
    if (funName === 'switchTab') {
      const route = routesForMapRoute(router, path, [
        'pathMap',
        'finallyPathList',
      ])
    ...
    const {
      ...
      animationType,
      animationDuration,
    } = option as uniNavApiRule
    const jumpOptions: totalNextRoute = {
      path,
      events,
      success,
      fail,
    ...
    router[routerMethodName](notDeepClearNull<totalNextRoute>(jumpOptions))
  }
}
